...
void MostraCarro(){
  ...
  GerarPlaca(placa);
  gerarFabricante(fabricante);
  gerarModelo(fabricante,modelo, preco_compra);
  ano_fabricacao=gerarAnoFabricacao(ano_fabricacao, preco_compra);
  gerarAnoModelo(ano_fabricacao,ano_modelo);
  gerarGasolina(combustivel);
  gerarCor(cor);
